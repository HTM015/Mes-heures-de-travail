...
const CACHE_NAME = 'mes-heures-klinkenberg-v4';
...
const FILES_TO_CACHE = [
  ...
  './icon-192.png',
  './icon-512.png',
  './klinkenberg-header.png',
  './klinkenberg-logo.png',
  ...
  event.waitUntil(
    caches.open(CACHE_NAME).then(cache => {
      return cache.addAll(FILES_TO_CACHE).catch(() => {
        ...
        return cache.addAll(['./', './index.html', './manifest.json', './icon.png', './icon-192.png', './icon-512.png', './klinkenberg-header.png', './klinkenberg-logo.png']);
      });
    })
  );
  self.skipWaiting();
